...
int main() {
	...
	int numTreasures = 0, drawnTreasure = 0, cardDrawn = 0, z = 0;
	int numFailed = 0;
	int adventurerPos[4];
	...
	for (numTest = 0; numTest < MAX_TESTS; numTest++) {
		...
		if(assert1(gameAfterCall.handCount[player] == game.handCount[player] + numTreasures, "error in adding treasures to hand", numTest) == 1) numFailed++;
